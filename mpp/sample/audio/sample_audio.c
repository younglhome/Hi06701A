...
HI_S32 SAMPLE_AUDIO_AdecAo(HI_VOID)
{
	...
	s32Ret = SAMPLE_COMM_AUDIO_StopAo(AoDev, stAioAttr.u32ChnCnt, gs_bAioReSample, HI_FALSE);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_DBG(s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_AUDIO_InnerCodecAiAo(HI_VOID)
{
    ...
    s32Ret = SAMPLE_COMM_AUDIO_StopAo(AoDev, s32AoChnCnt, (gs_pstAoReSmpAttr != NULL ? HI_TRUE : HI_FALSE), HI_FALSE);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_DBG(s32Ret);
        return HI_FAILURE;
...
HI_S32 SAMPLE_AUDIO_Tlv320CodecAiAoHDMI(HI_VOID)
{
    ...
    /* enable AO channle */
    s32AoChnCnt = stAioAttr.u32ChnCnt;
    s32Ret = SAMPLE_COMM_AUDIO_StartAo(AoDev, s32AoChnCnt, &stHdmiAoAttr, gs_pstAoReSmpAttr->enInSampleRate, HI_TRUE, NULL, 0);
    if (s32Ret != HI_SUCCESS)
    {
        SAMPLE_DBG(s32Ret);
        return HI_FAILURE;
