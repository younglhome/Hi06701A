...
(1)编译整个osdrv目录：
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3521a all
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3520dv300 all
	或者
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3521a all
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3520dv300 all

(2)清除整个osdrv目录的编译文件：
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3521a clean
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3520dv300 clean
	或者
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3521a clean
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3520dv300 clean
(3)彻底清除整个osdrv目录的编译文件，除清除编译文件外，还删除已编译好的镜像：
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3521a distclean
	make OSDRV_CROSS=arm-hisiv300-linux CHIP=hi3520dv300 distclean
	或者
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3521a distclean
	make OSDRV_CROSS=arm-hisiv400-linux CHIP=hi3520dv300 distclean

(4)单独编译kernel：
	待进入内核源代码目录后，执行以下操作
	cp arch/arm/configs/hi3521a_full_defconfig  .config
	cp arch/arm/configs/hi3520dv300_full_defconfig  .config
...
(6)单独编译uboot：
	...
	运行./mkboot.sh reg_info_hi3521a_400M_1GB_32bit_1100M_250M.bin u-boot-hi3521a.bin
	生成的u-boot-hi3521a.bin即为可用的u-boot镜像
	或者
	make ARCH=arm CROSS_COMPILE=arm-hisiv400-linux- hi3521a_config 
	make ARCH=arm CROSS_COMPILE=arm-hisiv400-linux- hi3520dv300_config 
...
在osdrv/pub/中有已经编译好的文件系统，因此无需再重复编译文件系统，只需要根据单板上flash的规格型号制作文件系统镜像即可。
	...
	spi nand flash使用yaffs2格式的镜像，制作yaffs2镜像时，需要用到nand flash的pagesize和ecc。这些信息会在uboot启动时会打印出来。建议使用时先直接运行mkyaffs2image工具，根据打印信息填写相关参数。下面以2KB pagesize、4bit ecc为例：
...
│      └─uboot_tools ---------------------------- uboot镜像制作工具、xls文件及ddr初始化脚本、Hitools工具
├─pub ------------------------------------------- 存放各种镜像的目录
...
(3)编译板端软件
    a.Hi3521A具有浮点运算单元和neon。文件系统中的库是采用软浮点和neon编译而成，因此请用户注意，所有Hi3521A板端代码编译时需要在Makefile里面添加选项-mcpu=cortex-a7、-mfloat-abi=softfp和-mfpu=neon-vfpv4。
    b.v300和v400工具链都是基于GCC4.8的。从GCC4.7开始，在编译基于ARMv6 (除了ARMv6-M), ARMv7-A, ARMv7-R, or ARMv7-M的代码时，为了支持以地址非对齐方式访问内存，默认激活了-munaligned-access选项，但这需要系统的内核支持这种访问方式。Hi3521A不支持地址非对齐的内存访问方式，所以板端代码在编译的时候需要显式加上选项-mno-unaligned-access。(http://gcc.gnu.org/gcc-4.7/changes.html)
